...
int physicalAddress (uint vAddr, char action)
{
	...
	if (page < 0 || page >= nPages) { 	// Invalid Page number
		return -1;
	}
	...
	if (PageTable[page].status.loaded == 1) {
		...
		int pAddr = PageTable[page].frameNo * PAGESIZE + offset;
		return pAddr;
	} else {
		uint unused_frame = -1;
		...
		if (unused_frame == -1) {
			...
			for (uint i = 0; i < nPages; i++) {
				if (PageTable[i].status.loaded == 1) {
					if (PageTable[i].lastAccessed < min_time) {
						lru = i;
						min_time = PageTable[i].lastAccessed;
					}
				}
			}

			PageTable[lru].status.loaded = 0;
			if(PageTable[lru].status.modified == 1){
				nSaves++;
			}

			PageTable[lru].status.modified = 0;
			unused_frame = PageTable[lru].frameNo;

			PageTable[lru].frameNo = -1;
			PageTable[lru].lastAccessed = -1;
			MemFrames[unused_frame] = -1;
		...
		MemFrames[unused_frame] = page;
		PageTable[page].frameNo = unused_frame;
